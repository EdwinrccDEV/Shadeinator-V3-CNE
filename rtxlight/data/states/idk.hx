	...
	},
	{
		...
		childs: [
			...
			{label: "Save shader as", onSelect: () -> trace("hi")},
			{
				label: "Make script",
				onSelect: () -> {
					var nombre = "mi_shader_script"; // nombre del archivo
					var round = true; // si quieres redondear
					var val = (v:Float) -> if (round) Math.round(v * 100) / 100 else v;
					// Construir contenido
					var contenido = "";
					contenido += "var sprite = strumLines.members[x].characters[y];\n";
